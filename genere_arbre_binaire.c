...
void parcours_infixe_2_prefixe_filiforme_aleatoire(int * prefixe, int * infixe, int n){
    if (n == 0)
        return;
    int rd = rand()%2;
    if (rd == 1){ //si on va à droite, la racine c'est le premier elem d'infixe
        prefixe[0] = infixe[0];
        parcours_infixe_2_prefixe_filiforme_aleatoire(prefixe+1, infixe+1, n-1);
    } else { //sinon pour gauche c'est le dernier elem, donc n-1
...
static int filiforme_alea(Arbre *a, int taille, int est_abr){
    int *infixe = malloc(sizeof(int)*taille);
    if (infixe == NULL)
        return -1;
    if (est_abr)
